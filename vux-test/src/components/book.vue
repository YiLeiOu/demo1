  ...
  <div class="app-book">
   <v-header title="分类列表"></v-header>
    <section class="container">
        <div class="search">
            <input autocomplete="off" class="s-value" v-model="searchValue" @keyup.enter="goSearch">
            <div class="search-btn" @keyup.enter="goSearch" @click="goSearch">搜索</div>
        </div>
...
export default {
  ...
  methods: {
    goSearch () {
      if (this.searchValue !== '') {
        this.$router.push({name: 'search_list', query: {q: this.searchValue}})
      } else {
        alert('请输入关键字！')
      }
    }
